...
import { NameFinder } from './intellisense/NameFinder';
	...
	if (vscode.workspace.getConfiguration('ruby').intellisense == 'rubyLocate') {
		const refreshLocate = () => {
			let progressOptions = {
				...
				title: 'Indexing Ruby source files',
			};
		...
		const locationConverter = match =>
			new vscode.Location(vscode.Uri.file(match.file), new vscode.Position(match.line, match.char));
		const defProvider = {
		...
		const defFullWordProvider = {
			provideDefinition: (doc, pos) => {
				const line = doc.lineAt(pos.line);
				const finder = new NameFinder(line.text, pos.character);
				return locate.find(finder.get).then(matches => matches.map(locationConverter));
			},
		};

		ctx.subscriptions.push(
			vscode.languages.registerDefinitionProvider(['ruby', 'erb'], defProvider)
		);

		ctx.subscriptions.push(
			vscode.languages.registerDefinitionProvider(['ruby', 'erb'], defFullWordProvider)
		);
		...
		const symbolKindTable = {
			...
			method: symbolInfo =>
				symbolInfo.name === 'initialize' ? SymbolKind.Constructor : SymbolKind.Method,
		...
		const symbolsConverter = matches =>
			matches.slice(0, numOfSymbolLimit).map(match => {
				const symbolKind = (symbolKindTable[match.type] || defaultSymbolKind)(match);
				return new SymbolInformation(
					match.name,
					symbolKind,
					match.containerName,
					locationConverter(match)
				);
			});
		...
		ctx.subscriptions.push(
			vscode.languages.registerDocumentSymbolProvider(['ruby', 'erb'], docSymbolProvider)
		);
		...
		var rubyLocateDisabled = () => {
			vscode.window.showInformationMessage(
				'The `ruby.intellisense` configuration is not set to use rubyLocate.'
			);
		};
		ctx.subscriptions.push(
			vscode.commands.registerCommand('ruby.reloadProject', rubyLocateDisabled)
		);
	}
}
